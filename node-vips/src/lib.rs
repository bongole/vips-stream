...
use napi::{
    ...
    CallContext, Env, JsBuffer, JsBufferValue, JsExternal, JsFunction, JsObject, JsUndefined, Ref,
    Result, Task,
};
...
#[js_function(1)]
fn create_vips_image(ctx: CallContext) -> Result<JsObject> {
...
#[js_function(2)]
fn register_read_buf(ctx: CallContext) -> Result<JsUndefined> {
...
#[js_function(1)]
fn register_read_end(ctx: CallContext) -> Result<JsUndefined> {
    let ctx_obj = ctx.get::<JsExternal>(0)?;
    let native = ctx
        .env
        .get_value_external::<Arc<ReadContextNative>>(&ctx_obj)?;
...
fn init(mut exports: JsObject) -> Result<()> {
    ...
    exports.create_named_method("createVipsImage", create_vips_image)?;
    exports.create_named_method("registerReadBuf", register_read_buf)?;
    exports.create_named_method("registerReadEnd", register_read_end)?;
